...
public class LoginGenerator {
     ...
     * premiere lettre du prenom, concatenee avec
     * les 3 premieres lettres du nom, le tout mis en lettres capitales et
     * desaccentue.
     * Le login genere doit etre unique
     * par rapport a la liste des logins existants. En cas de doublon, on
     * incremente le doublon d'un indice. Ci dessous des
     * exemples :
     * <ul>
    ...
    public String generateLoginForNomAndPrenom(String nom, String prenom) {
        ...
        if (loginService.loginExists(login)) {
            ...
            int nbLoginSimilaire = loginService.findAllLoginsStartingWith(login)
                    .size();


            login = login + String.valueOf(nbLoginSimilaire);
        }
